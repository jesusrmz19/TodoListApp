...
class TodoView extends View {
  ...
  toggleLoginLogout() {
    this._btnLogin.classList.toggle('hidden');
  ...
  _generateSingleMarkup() {
    return `
            <li class="todo--item todo-${this._data.id} " >
                <input type="checkbox" class="hidden-box" id="todo-${
                  this._data.id
                }" />
                <label class="todo--label ${
                  this._data.checked ? 'checked' : ''
                }" for="todo-${this._data.id}">
                <span class="check--box"></span>
                <span class="check--text">${this._data.value}</span>
                <button class="btn--remove btn--${this._data.id}" data-todo="${
      this._data.id
    }" aria-label="Delete Todo Button"></button>
                </label>
            </li>
        `;
  }
      ...
      .map((todo) => {
        return `
            <li class="todo--item todo-${todo.id} " >
                ...
                <button class="btn--remove btn--${todo.id}" data-todo="${
          todo.id
        }" aria-label="Delete Todo Button"></button>
                </label>
            </li>
